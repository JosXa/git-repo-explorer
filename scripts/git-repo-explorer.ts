...
const data = await db({ accessToken: null, recentRepos: [] as RepoInfo[] });

if (!data.accessToken) {
  open('https://github.com/settings/personal-access-tokens/new');
  data.accessToken = await env(
    'GITHUB_PAT',
    'Personal, fine-grained GitHub access token with public repository read access',
  );
  await data.write();
}
...
const repoInfo = await arg<RepoInfo>({
  ...
  choices: async (input) => {
    ...
    return [...searchResults, ...cachedResults].map((c) => ({
      ...c,
...
onTab('File Search', async () => {
  const allFilesResponse = await octokit.request('GET /repos/{owner}/{repo}/git/trees/{tree_sha}?recursive=1', {
    owner,
    repo,
    tree_sha: 'main', // TODO: Make it possible to check for `next` branch docs
    headers: {
...
onTab('Code Search', async () => {
  await arg('Search Files', async (input) => {
    ...
    const searchResults = await octokit.rest.search.code({
      q: `${input}+in:file+language:ts+repo:${repoId}`,
